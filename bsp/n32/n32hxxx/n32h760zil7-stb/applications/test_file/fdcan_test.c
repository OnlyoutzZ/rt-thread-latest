...
static void _fdcan_internal_loopback(const char *name)
{
    ...
    {
        tx_msg.data[i] = i + (rt_uint8_t)(name[5] - '0');
    }
    ...
    for (rt_uint8_t i = 0; i < rx_msg.len && i < 64; i++)
    {
     ...
     * 5Mbps (that needs a real wired peer, see fdcan5<->6 testing). */
    {
        ...
        {
            fd_tx_msg.data[i] = (rt_uint8_t)(i + (name[5] - '0'));
        }
...
static rt_bool_t _fdcan_basic_pair(const char *name_a, const char *name_b)
{
    ...
    if (!_fdcan_send_and_check(dev_a, dev_b, 0x100, 8, 0, 0, tag))
    {
        pass = RT_FALSE;
    }
    rt_snprintf(tag, sizeof(tag), "%s->%s classic", name_b, name_a);
    if (!_fdcan_send_and_check(dev_b, dev_a, 0x101, 8, 0, 0, tag))
    {
        pass = RT_FALSE;
    }
    ...
    if (!_fdcan_send_and_check(dev_a, dev_b, 0x102, 64, 1, 1, tag))
    {
        pass = RT_FALSE;
    }
    rt_snprintf(tag, sizeof(tag), "%s->%s fd+brs@4M", name_b, name_a);
    if (!_fdcan_send_and_check(dev_b, dev_a, 0x103, 64, 1, 1, tag))
    {
        pass = RT_FALSE;
    }
...
static rt_bool_t _fdcan_fd_rate_pair(const char *name_a, const char *name_b, rt_uint32_t fd_baud)
{
    ...
    else
    {
        ...
        if (!_fdcan_send_and_check(dev_a, dev_b, 0x110, 64, 1, 1, tag))
        {
            pass = RT_FALSE;
        }
        rt_snprintf(tag, sizeof(tag), "%s->%s fd@%dM", name_b, name_a, fd_baud / 1000000);
        if (!_fdcan_send_and_check(dev_b, dev_a, 0x111, 64, 1, 1, tag))
        {
            pass = RT_FALSE;
        }
    }
...
static rt_bool_t _fdcan_filter_pair(const char *name_a, const char *name_b)
{
     ...
     * path asserts hdr_index is within [0, maxhdr) when RT_CAN_USING_HDR is on. */
    tx_msg.id = 0x301;
    tx_msg.data[0] = 0xBB;
    ret = rt_device_write(dev_a, 0, &tx_msg, sizeof(tx_msg));
    if (ret != sizeof(tx_msg))
