 ...
 * ============================================================================
 *
 *  1. Read the current RTC time (quick check that the RTC is running):
 *         rtc_test_info
 *         date
 *      Run it twice, 2~3 seconds apart: a rising second count means the RTC
 *      is keeping time.
 *
 *  2. Set / query the time:
 *         date                         # query only (no arguments)
 *         date 2026 08 14 12 30 00     # set: year month day hour min sec (local, spaces required)
 *      Note: the RT-Thread date command interprets its input as local time and
 *      stores it in the RTC.
 *      This driver stores RTC hardware time as UTC, and adds +8h for local
 *      display when querying, so after setting 12:30 a query returns 12:30
 *      (same value).
 *      After the first flash or a chip swap the RTC is uninitialised (no magic
 *      0xA5A5 in the backup register); run the date set command above once
 *      before running the tests.
 *
 *  3. Run every test at once (recommended):
 *         rtc_test_full
 *      A final "pass: N, fail: 0" means everything passed; if fail > 0 look at
 *      the individual [FAIL] items.
 *
 *  4. Per-module tests:
 *         rtc_test_init_cmd     # driver init / clock configuration only
 *         rtc_test_time_cmd     # time read/write only (secs/date/timeval)
 *         rtc_test_alarm_cmd    # alarm only (set/get/daemon/modify/delete/rearm)
 *         rtc_test_backup_cmd   # backup register read/write only
 *
 *  5. Alarm helpers:
 *         list_alarm            # RT-Thread's built-in alarm list command
 *
 *  Notes:
 *  - The tests overwrite the RTC time (groups 08/09 set it to a test baseline);
 *    re-run date afterwards if you need the real time.
 *  - The backup register test reads and writes RTC_BKP_REG1 (0x5A5 restored to
 *    0xA5A5 afterwards) and does not touch the driver magic.
 *  - This file requires BSP_USING_ONCHIP_RTC (rtconfig.h); the alarm tests also
 *    require RT_USING_ALARM.
 *
