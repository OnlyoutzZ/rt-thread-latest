# N32H487ZGL7-EVB CherryUSB (FSDEV_NATION) HID Keyboard Test Guide

## 1. System Clock (HSE)

`RCC_Configuration()` in `board/Cube_Config/USER/src/n32h47x_48x_cfg.c` uses **HSE as the PLL source**:
...
Device information: VID/PID = `0xffff/0xffff`, product name "N32H487 EVB HID Keyboard".
...
- When a key is pressed (pull-up input, active low), the `key_scan` thread (10 ms polling + 10 ms debounce) detects the edge transition and sends a keyboard report; multi-key combinations are supported (up to 4 keys in one report).
...
2. Connect the board's USB port to the PC with a USB cable. The PC should enumerate a HID keyboard device
   ("N32H487 EVB HID Keyboard", listed under the "Keyboards" category in Device Manager).
...
4. Press CapsLock on the host keyboard:
   - Expected: on-board LED2 (PB4) lights up; press CapsLock again -> LED2 turns off.
...
| Symptom | Check |
| --- | --- |
| PC does not enumerate the device | Check the PA11/PA12 wiring; verify the USBFS 48 MHz clock (PLL/5 when HSE=8 MHz); check the serial boot log |
